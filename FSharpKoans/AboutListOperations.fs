    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int =
            let rec getLength xs acc =
                match xs with
                | [] -> acc
                | _::tail -> getLength tail (acc+1)
            getLength xs 0
    ...
    let ``03 Reversing a list, the hard way`` () =
        let rev (xs : 'a list) : 'a list =
            let rec this (xs : 'a list) (ys : 'a list) : 'a list =
                match xs with
                | [] -> ys
                | first::tail -> this tail (first::ys)
            this xs []// write a function to reverse a list here.
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            let rec addOne xs ys =
                match xs with 
                | [] -> List.rev ys
                | first::tail -> addOne tail (first+1::ys)
            addOne xs [] // write a function which adds 1 to each element
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
            let rec double xs ys =
                match xs with 
                | [] -> List.rev ys
                | first::tail -> double tail (first*2::ys)
            double xs []// write a function which doubles each element
   ...
   *)
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
            let rec startMap f xs ys =
                match xs with 
                | [] -> List.rev ys
                | head::tail -> startMap f tail (f(head)::ys)
            startMap f xs []// write a map which applies f to each element
    ...
    let ``09 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            let rec startFilter f xs ys = 
                match xs with
                | [] -> List.rev ys
                | head::tail -> match f(head) with
                                | false -> startFilter f tail ys
                                | _ -> startFilter f tail (head::ys)
            startFilter f xs [] // write a function which filters based on the specified criteria
        ...
        let filter (xs : int list) : int list =
            let rec odd xs ys = 
                ...
                | head::tail -> match head%2 = 0 with 
                                | true -> odd tail ys
                                | _ -> odd tail (head::ys)
            odd xs []// write a function to filter for odd elements only.
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
            let rec startFold xs acc =
                match xs with
                | [] -> acc
                | head::tail -> startFold tail acc+head
            startFold xs initialState// write a function to do what's described above
    ...
    let ``15 A fold which multiplies a list`` () =
        let fold initialState xs =
            let rec startFold xs acc =
                ...
                | [] -> acc
                | head::tail -> startFold tail acc*head
            startFold xs initialState // write a function to multiply the elements of a list
    ...
    let ``16 Folding, the hard way`` () =
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            let rec startFold f xs acc =
                match xs with
                | [] -> acc
                | head::tail -> startFold f tail (f acc head)
            startFold f xs initialState// write a function to do a fold.
        fold (+) 0 [1;2;3;4] |> should equal 10
    ...
    let ``18 exists: finding whether any matching item exists`` () =
        let exists (f : 'a -> bool) (xs : 'a list) : bool =
            let rec checkExists f xs is=
                match is with
                | true -> is
                | _ -> match xs with
                       | head::tail -> checkExists f tail (f head)
                       | [] -> is
            checkExists f xs false// Does this: https://msdn.microsoft.com/en-us/library/ee370309.aspx
    ...
    let ``19 partition: splitting a list based on a criterion`` () =
        let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) =
            let rec split f xs l1 l2 =
                match xs with 
                | [] -> List.rev l1, List.rev l2
                | head::tail -> match f(head) with
                                | true -> split f tail (head::l1) l2
                                | _ -> split f tail l1 (head::l2)
            split f xs [] []// Does this: https://msdn.microsoft.com/en-us/library/ee353782.aspx
    ...
    let ``20 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
            let rec make f n xs acc=
                match n > acc with
                | false -> List.rev xs
                | _ -> make f n ((f acc)::xs) (acc+1)
            make f n [] 0// Does this: https://msdn.microsoft.com/en-us/library/ee370497.aspx
        ...
        let tryFind (p : 'a -> bool) (xs : 'a list) : 'a option =
            let rec find p xs =
                ...
                | head::tail -> match (p head) with
                                | true -> (Some head)
                                | _ -> find p tail
            find p xs // Does this: https://msdn.microsoft.com/en-us/library/ee353506.aspx
    ...
    *)
    ...
    let ``23 choose: find all matching elements, and transform them`` () =
        ...
        let choose (p : 'a -> 'b option) (xs : 'a list) : 'b list =
            let rec startChoose p xs ys =
                match xs with
                | [] -> List.rev ys
                | head::tail -> match (p head) with
                                | None -> startChoose p tail ys
                                | (Some n) -> startChoose p tail (n::ys)
            startChoose p xs []// Does this: https://msdn.microsoft.com/en-us/library/ee353456.aspx
        let f x =
            match x<=45 with
            | true -> Some(x*2)
            | _ -> None
    ...
    let ``24 mapi: like map, but passes along an item index as well`` () =
        let mapi (f : int -> 'a -> 'b) (xs : 'a list) : 'b list =
            let rec startMapi f xs ys acc= 
                match xs with
                | [] -> List.rev ys
                | head::tail -> startMapi f tail ((f acc head)::ys) (acc+1)
            startMapi f xs [] 0// Does this: https://msdn.microsoft.com/en-us/library/ee353425.aspx
